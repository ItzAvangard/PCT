...
def draw(filenames, labels, colors):
    ...
    ax.set_title("")
    ax.set(xlabel="$p$ count", ylabel="Speedup")
    ax.label_outer()
    ...
    for (fname, datalabel, color) in zip(filenames, labels, colors):
        ...
        #x.bar(x, y);
    ax.plot(range(2,12),range(2,12),'-',c="blue",linewidth=0.5,label="Linear speedup")
...
if __name__ == "__main__":
    draw(["TH1000.txt", "TH10000.txt"], ["TH1000","TH10000"], ["red", "green"])